<template>
  <div :class="b({ card: !option.card })">
    <component :is="tableOption.titleSize || 'h2'" v-if="tableOption.title" :style="tableOption.titleStyle">{{
      tableOption.title
    }}</component>
    <!-- 搜索组件 -->
    <el-card :shadow="isCard">
      ...
      <el-form ref="cellForm" :model="cellForm" :show-message="false" @validate="handleValidate">
        <el-table
          v-if="reload"
          :data="cellForm.list"
...
export default {
  ...
  methods: {
    ...
    vaildParent(row) {
      return validatenull(row[this.rowParentKey])
    },
    // 删除
    rowDel(row, index) {
